...
export default function Notification({
  ...
  details,
  onClose,
  autoCloseDelay = 4000 // 4 segundos por padrão
}) {
  ...
  useEffect(() => {
    if (autoCloseDelay && typeof onClose === 'function') {
      const timer = setTimeout(() => { // Adicione arrow function
        onClose(); // Execute a função
      }, autoCloseDelay);

      return () => clearTimeout(timer);
    }
  }, [onClose, autoCloseDelay]); // Dependências corretas
